...
$$K(x_f, t_f ; x_i, t_i) = \langle x_f | \hat{U}(t_f, t_i) | x_i \rangle = \underbrace{\lim_{\substack{{N \to \infty} {\epsilon \to 0}}} \Big(\frac{m}{2 \pi i \epsilon}\Big)^{N/2} \int \prod_{n=1}^{N-1} d x_n}_{\int \mathcal{D}[x(t)]}\: e^{iS[x(t)]} = \int \mathcal{D}[x(t)] \: e^{iS[x(t)]}$$
